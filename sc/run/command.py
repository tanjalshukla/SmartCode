...
def run(
    ...
    # Stage 1: obtain an intent declaration, resolving any model check-ins/read requests first.
    while declaration is None:
        ...
        # check in logic, transition to next phase of exec
        ...
        if checkpoint.required:
            ...
            if plan_feedback:
                session.add_memory_note(f"Plan guidance: {plan_feedback}")
    ...
    if show_intent and not intent_rendered_during_checkpoint:
    ...
    try:
        updates, patch_text, touched_files, current_phase = _generate_updates_with_repair(
            client=client,
            session=session,
